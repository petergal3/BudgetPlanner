with saving_s as (select sum (amount) as s, s.date as d from dbo.saving s where s.date between '2021-01-01' and '2021-12-30'  group by s.date),
transactions_s as ((select sum(amount) as t1, t.date as d from dbo.transactions t where type = 'income' and t.date between '2021-01-01' and '2021-12-30' group by t.date))
